...
import { SearchPageController } from './searchpage.component';
let controller: SearchPageController;
let $componentController: angular.IComponentControllerService;
let $rootScope: angular.IRootScopeService;
let $q;
const MovieService = jasmine.createSpyObj('app', ['getMovie']);
...
beforeEach(() => {
    ...
    function createController(): SearchPageController {
        return $componentController('app', { $scope: $rootScope.$new() }, {}) as SearchPageController;
    }
    it('should get data from service', () => {
        const ctrl = createController();
        ctrl.getMovie('Batman');
        $rootScope.$apply();
        expect(ctrl.searchedMovieList).toEqual(stub);
    });
});
